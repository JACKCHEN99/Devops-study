...
chk_health() {
    while read line;do
        ...
        if [[ $? -eq 0 ]]; then
            ...
            ls /data/script/k8s-uatstable/yaml/$line &>/dev/null
            [ $? -eq 0 ]  && echo "$line not_found no_yaml" >> result.txt || echo "$line no_found yaml_yes no_yaml" >> result.txt
        fi
    done< $1.txt
    echo
    ...
    cat result.txt | awk '/no_found/{print $1,$3}' | column -t
}
